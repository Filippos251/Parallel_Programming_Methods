...
double f(double *x, int n)
{
	...
	#pragma omp atomic
    funevals++; // Ασφαλής αύξηση από πολλαπλά νήματα
    fv = 0.0;
...
int main(int argc, char *argv[])
{
	...
	// Διαχείριση όλων των best_* δεδομένων στο critical
    #pragma omp parallel for private(trial, i, startpt, endpt, fx, nt, nf)
	for (trial = 0; trial < ntrials; trial++) {    
		srand48(trial);// Κάθε trial έχει το δικό του seed
